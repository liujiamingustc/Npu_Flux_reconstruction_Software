module metrics_mod
  ...
  !           NOTE: met is actually the inverse of the jacobian matrix.
  !                 jac(n) in this data structure is the determinant of the
  !                 jacobian matrix at the solution poin n.
  !                 The jacobian is abs of the determinant of the jacobian
  !                 matrix, i.e. the derivative matrix.
...
continue
  ...
  ! Here, met is the jacobian matrix. Later, met is modified.
  !
 ...
 !end do
  ...
  ! The function reverse_metrics is a way of calculating the inv of the matrix.
  ! But the direct output is not exactly the same as the inv matrix. Only after
  ! the jac is divided, the output is the inv matrix.
  ! The reverse_metrics function utilizes the cross_product. I did not try to
  ! figure out why. But it should be reasonable for calculating the inv.
  !
  do n = 1,np
