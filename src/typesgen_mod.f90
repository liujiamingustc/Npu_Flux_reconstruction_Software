...
continue
 ...
 !this%mmut_rev_mult => mm_rev_mult_intsc_use_transpose
  ...
  if (allocated(this%mat)) then
    ...
    ! For the format of the sparse matrix storage, check the Wiki:
    ! https://en.wikipedia.org/wiki/Sparse_matrix#Compressed_sparse_row_(CSR,_C
    ! RS_or_Yale_format)
    ! ######################################
    ! ######################################
    ! ######################################
    ...
    if (sparsity <= sparsity_limit) then
      ...
      if (use_unrolled_dot_products) then
        ...
        if (n1 == n2) then
          ...
          ! False. Manually unrolled loop is not fit for all loops.
        else
          !
          this%dot => dot_sparse
          this%dot_mat => dot_mat_sparse
        ...
        ! User choose not to use the manual unrolled loop.
        this%dot => dot_sparse
        this%dot_mat => dot_mat_sparse
        this%dot_mat2 => dot_mat2_sparse
        !
...
continue
  ...
  n3 = n2+1
  n4 = n3+1
  ! This is actually the dot product of non-zeros in idx column of the sparse
  ! matrix and its associated elements in n column of the array for each n.
  ! This is no longer the common sense of the matrix-vector multiplication.
  do n = 1,size(array,dim=2)
    return_value(n) = this%row_val(n1) * array( this%row_idx(n1) , n ) + &
                      this%row_val(n2) * array( this%row_idx(n2) , n ) + &
                      this%row_val(n3) * array( this%row_idx(n3) , n ) + &
