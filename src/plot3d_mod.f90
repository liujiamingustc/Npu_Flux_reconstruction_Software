...
continue
  ...
  call alloc_error(pname,"bcs_info",1,__LINE__,__FILE__,ierr,error_message)
  !
  ! TODO: merge bc_names, bcs_info into bc_in. And bc_in is globally accessible.
 ...
 !read (iobcs,1,iostat=ierr,iomsg=error_message) text
  ...
  do nb = 1,nbc
   ...
   !read (iobcs,1,iostat=ierr) text
    ...
    !
    ! Set bc_type in bc_in, otherwise it cannot be set anywhere else.
    ! Should be set other stuff in bc_in in this function? TODO
    bc_in(nb)%bc_type = bcs_info(nb)%bc_type
    !
